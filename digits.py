...
from utils import preprocess_data, tune_hparams, split_train_dev_test,read_digits,predict_and_eval,scale_image
...
for dimension in image_dimension:
    ...
    for test_size in test_sizes:
        for dev_size in dev_sizes:
            # 3. Data splitting
            X_train, X_test,X_dev, y_train, y_test,y_dev = split_train_dev_test(x_resize, y, test_size=test_size, dev_size=dev_size);
            ...
            print(f"image_size={dimension}x{dimension},test_size={test_size} dev_size={dev_size} train_size={1- (dev_size+test_size)} train_acc={accuracy_train} dev_acc={accuracy_dev} test_acc={accuracy_test}")
